...
/// 用户协议页。
///
/// 版本号来自服务端 `GET /api/v1/auth/agreements`，与登录时提交的 agreementVersion
/// 同源，保证「用户看到的版本」与「服务端留痕的版本」一致。
...
class _AgreementPageState extends State<AgreementPage> {
  ...
  Future<void> _loadVersion() async {
    final versions = await AuthScope.of(context).loadAgreementVersions();
    if (!mounted) {
      return;
    }
    setState(() => _version = versions.userAgreementVersion);
  }
